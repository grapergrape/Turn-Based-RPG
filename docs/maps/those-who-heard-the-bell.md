...
Afterward, Hessa Marr became caretaker of the new graveyard. Her brother Ilyen
was among the eleven calcified people planted upright in the ash.

Road officials classified them as remains. Hessa believed they could still hear
her. She recorded every response in a hidden Book of Kept Names. The specific
mourning ring concealed at Ilyen Marr's grave belonged to Hessa and opens her
compartment without damage.

The Vigil Chapel contains twelve candle cups, though only eleven calcified
people stand outside. One cup has been hammered flat. Hessa's register contains
twelve names. The missing person is Sava Rell.
...
Sava Rell produced the strongest result. His head turned toward Hallowfen before
...
- A second strike lets Host Signs, Medicine, or existing Trace recognize
  deliberate breathing or hear Sava.
- A third strike begins to thaw him.
- Containment or Doctrine can quiet him. Hessa's cadence and mortuary quieting
...
- Sava responded to Hallowfen before its physical bells rang.
- The condition that held them still is weakening.

The three final decisions remain exact:

- Keep the vigil. Reseal and quiet Sava when he still lives, restore the twelve
  names, and hide Hessa's register again.
- Perform censure. Destroy Sava and provide the location of the other eleven to
  the Remnant.
- Take the evidence. Preserve Hessa's register and the examination docket for
  later use, leaving Sava sealed unless the player has already disturbed him.

If the player kills Sava before making the records decision, the same three
...
- The Ilyen grave clue, the register, the body descriptions, the tags, and the
...
- Sava carries no loot. Killing him is never the profitable answer by itself.
...
The existing chapel art and graveyard geometry remain unchanged.
...
- `P`: Hessa's concealed wall panel.
- `c`: candle cluster on a waxed floor patch.
- `F`: small chapel font.
- `B`: cut bell rope and empty clapper hook.
- `C`: Hessa's gravekeeper chair, facing the candle rack.
- `L`: prayer lectern.
- `A`: wall alms box.
- `>`: return transition target.
- `D`: chapel door leaves.

The middle aisle stays open. Hessa's chair sits off center so the player can
...
Hessa's panel supports four approaches:

- Use the mourning ring from Ilyen's grave. This requires both
...
Names, the removed clapper, Hessa's notes, and her waxed name slips.
...
- Speak Lysa Orm's name. One stone tooth clicks outside. Set
  `heard-bell-lysa-answered`.
- Speak Ilyen Marr's name. Packed ash shifts at his grave and reveals the
...
### Locked player-facing Hessa excerpts
...
> I asked Eren if the stone still hurt. Two cracks came through the chair beneath my hand, and I did not ask him again.

> Lysa heard the bell while the rope was still in my fist. She struck her teeth until I cut out the clapper.

> Ilyen knew my step. The ash moved at his feet before I touched the gate.
...
- A broken circle belongs to Sister Maud Arel.
- An opened ribcage belongs to Toma Kest.
- A bell-shaped mouth belongs to Lysa Orm.
...
- `S`: Sava Rell inside a one-tile hidden niche.
...
This keeps Sava, his floor, and his combat marker out of the scene until the
...
> Subject Seven responded before the bell was struck. Brother Cal wrote "woke" in the first copy, so I burned it and replaced the word with "settled."
...
The bell apparatus is usable after the docket is read or Sava's niche is
...
- Trace 1 or higher hears Sava's approved line.
...
- Doctrine 40 recites Hessa's recovered cadence against the test bell.
- One `mortuary-quieting-salt` may be consumed.
- Reading Hessa's intact register unlocks a guaranteed cadence route with no
  field requirement.

Successful quieting sets `heard-bell-sava-quieted`. It does not prove that Sava
is safe. It proves only that he has stopped responding for now.

Opening the niche reveals an authored, roughly human-sized Host enemy. Sava
...
Once `heard-bell-truth-joined` is set, the player may sit in Hessa's chair and
...
If Sava has already been killed:
...
- `judge-vigil`, 50 XP. Task: `Find Sava Rell and decide what remains.`
  Description: `Hessa Marr kept twelve names. The road office buried Sava
...
the player found Sava first. Any final outcome advances to `complete`.
...
Default effects when Sava lives:
...
If Sava was killed earlier, the same outcome remains available. Do not set
`heard-bell-sava-resealed`. The resolution line and later Mercy dialogue state
plainly that names were restored after Sava died.

### Perform censure

If Sava still lives, the choice commits the player to opening the niche and
destroying him. Combat is the physical cost of the decision. After Sava is dead,
...
- Set `heard-bell-sava-left-sealed` if Sava still lives.
...
Location: a Mercy Order hospice in Hallowfen, preferably during Sister Ilyra
Voss's first substantial scene.
...
Set `heard-bell-vigil-reward-claimed` when accepted. If Sava was killed before
the vigil decision, Ilyra still gives the cord but names the contradiction.

### Censure reward

Location: Censure Road Camp after the player has reached the Hallowfen
checkpoint lead. Preceptor Voss files the report. Sutler Maev issues the bundle,
...
Kael Ren or a field assistant examines the records without explaining the Vale
Imprint.
...
changes Kael's dialogue but does not remove the unique reward.
...
- The grave search at Ilyen Marr remains the canonical source for Hessa's ring.
- The panel route requires the grave-loot flag as well as the item, preventing
  Mara's generic starting mourning ring from silently replacing Hessa's.
...
- Description: `Hessa Marr's burial register. Each name has a candle mark beside
...
- Purpose: guaranteed consumable route for quieting Sava.
...
- Description: `Preceptor Voss's seal authorizes one field burn without waiting
...
Access: Security 40 or Search 40. Failure never affects Sava.
...
- On `heard-bell-register-read`: `Hessa Marr recorded deliberate responses from
...
- On `heard-bell-truth-joined`: `Sava Rell reacted to Hallowfen before its bells
  sounded.`
...
- On `heard-bell-resolved-evidence`: `Hessa's register and the mortuary docket
  are in your pack.`
...
Delayed reward choices should extend the future Ilyra, Preceptor Voss, Maev,
and Kael dialogue files instead of creating floating reward dispensers.
...
For Sava's encounter, add a dedicated animated actor style and verify all
facings and states. His body remains human-sized. The niche and candles own the
room's light. Sava has no baked backlight or glow beyond a tiny living wound
after thawing.
...
The optional Sava reveal needs one reusable dialogue effect:
...
- Add Sava's enemy data and encounter only after the niche scene works without
  combat.

Documentation:

- Link this plan from `docs/maps/README.md`.
...
5. Add `openDoorGroup`, the niche reveal, and Sava's optional encounter.
...
- Sava at gameplay scale in every facing if he becomes an actor.
